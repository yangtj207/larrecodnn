...
namespace nnet {
                    ...
                    art::ServiceHandle<art::TriggerNamesService const>()->getProcessName())
  {
    ...
    fPointIdAlgTool = art::make_tool<PointIdAlgTools::IPointIdAlg>(
      config.get_PSet().get<fhicl::ParameterSet>("PointIdAlg"));
  }
  // ------------------------------------------------------
  ...
  {
    mf::LogVerbatim("EmTrackMichelIdTl")
    ...
    if (fDoClusters) {
      ...
      auto cluID =
        fMVAWriter.initOutputs<recob::Cluster>(fNewClustersTag, fPointIdAlgTool->outputLabels());
          ...
          for (auto const& pview : ptpc.second) {
            ...
            for (size_t c : pview.second) // c is the Ptr< recob::Cluster >::key()
            {
              ...
              for (auto const& hit : v) {
                if (hitUsed[hit.key()]) {
                  mf::LogWarning("EmTrackMichelIdTl") << "hit already used in another cluster";
                }
    ...
    if (fDoTracks) {
      ...
      for (size_t t = 0; t < trkListHandle->size(); ++t) {
        ...
        while (!isViewSelected(best_view)) {
          ...
          if (++k > 3) {
            throw cet::exception("EmTrackMichelIdTl") << "No views selected at all?" << std::endl;
          }
        }
  ...
  bool
  EmTrackMichelIdTl::isViewSelected(int view) const
  {
    if (fViews.empty())
      return true;
    else {
