...
// File:        EmTrackMichelIdTl_module.cc (tool interface ver. of EmTrackMichelId)
...
EmTrackMichelIdTl::EmTrackMichelIdTl(EmTrackMichelIdTl::Parameters const& config) :
    EDProducer{config},
	    ...
	    art::ServiceHandle<art::TriggerNamesService const>()->getProcessName())
{
    ...
    // PointIdAlgorithm tool
    fPointIdAlgTool = art::make_tool<PointIdAlgTools::IPointIdAlg>(config.get_PSet().get<fhicl::ParameterSet>("PointIdAlg"));
...
{
    mf::LogVerbatim("EmTrackMichelIdTl") << "next event: " << evt.run() << " / " << evt.id().event();
    ...
    // ********************* classify hits **********************
    auto hitID = fMVAWriter.initOutputs<recob::Hit>(fHitModuleLabel, hitPtrList.size(), fPointIdAlgTool->outputLabels());
        ...
        for (auto const & ptpc : pcryo.second)
        {
            ...
            for (auto const & pview : ptpc.second)
            {
                ...
                for (size_t idx = 0; idx < pview.second.size(); idx += fBatchSize)
                {
                    ...
                    {
                        throw cet::exception("EmTrackMichelIdTl") << "hits processing failed" << std::endl;
                    }
    ...
    if (fDoClusters)
    {
        ...
        auto cluID = fMVAWriter.initOutputs<recob::Cluster>(fNewClustersTag, fPointIdAlgTool->outputLabels());
            ...
            for (auto const & ptpc : pcryo.second)
            {
                ...
                for (auto const & pview : ptpc.second)
                {
                    ...
                    for (size_t c : pview.second) // c is the Ptr< recob::Cluster >::key()
                    {
                        ...
                        {
                            if (hitUsed[hit.key()]) { mf::LogWarning("EmTrackMichelIdTl") << "hit already used in another cluster"; }
	    	            ...
	    	            mf::LogVerbatim("EmTrackMichelIdTl") << "cluster in tpc:" << tpc << " view:" << view
                    ...
                    for (size_t h : hitMap[cryo][tpc][view]) // h is the Ptr< recob::Hit >::key()
                    {
		                ...
		                mf::LogVerbatim("EmTrackMichelIdTl") << "single hit in tpc:" << tpc << " view:" << view
                    ...
                    mf::LogVerbatim("EmTrackMichelIdTl") << "...produced " << cidx - pview.second.size() << " single-hit clusters.";
                }
            }
        }
    ...
    if (fDoTracks)
    {
        ...
        for (size_t t = 0; t < trkListHandle->size(); ++t)
        {
            ...
            while (!isViewSelected(best_view))
            {
                ...
                if (++k > 3) { throw cet::exception("EmTrackMichelIdTl") << "No views selected at all?" << std::endl; }
            }
        ...
        auto trkID = fMVAWriter.initOutputs<recob::Track>(fTrackModuleLabel, trkHitPtrList.size(), fPointIdAlgTool->outputLabels());
...
bool EmTrackMichelIdTl::isViewSelected(int view) const
{
