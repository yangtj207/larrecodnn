...
EmTrack<N>::make_clusters(art::Event& evt, 
                           ...
                           EmTrack::cryo_tpc_view_keymap const & hitMap)
    {
        ...
        auto cluID = fMVAWriter.template initOutputs<recob::Cluster>(fNewClustersTag, fPointIdAlgTool->outputLabels());
          ...
          // clang-format on
                   if (!isViewSelected(view)) continue; // should not happen, clusters were pre-selected
                    ...
                    for (size_t c : clusters_keys) // c is the Ptr< recob::Cluster >::key()
                    {
                        ...
                        {
                            if (hitUsed[hit.key()]) { mf::LogWarning("EmTrack") << "hit already used in another cluster"; }
                            hitUsed[hit.key()] = true;
                        }

                        auto vout = fMVAWriter.template getOutput<recob::Hit>(v,
	    	            ...
	    	            fMVAWriter.template addOutput(cluID, vout); // add copy of the input cluster
                    }
                    ...
                    for (size_t h : hitMap.at({cryo,tpc,view})) // h is the Ptr< recob::Hit >::key()
                    {
		                ...
		                fMVAWriter.template addOutput(cluID, vout); // add single-hit cluster tagging unclutered hit
                    }
                    mf::LogVerbatim("EmTrack") << "...produced " << cidx - clusters_keys.size() << " single-hit clusters.";
        }
...
///make tracks
    template<size_t N>
    void
    EmTrack<N>::make_tracks(art::Event const & evt, std::vector< char > const & hitInFA)
    {
        ...
        for (size_t t = 0; t < trkListHandle->size(); ++t)
        {
            ...
            while (!isViewSelected(best_view))
            {
                ...
                if (++k > 3) { throw cet::exception("EmTrack") << "No views selected at all?" << std::endl; }
            }
        ...
        auto trkID = fMVAWriter.template initOutputs<recob::Track>(fTrackModuleLabel, trkHitPtrList.size(), fPointIdAlgTool->outputLabels());
        for (size_t t = 0; t < trkHitPtrList.size(); ++t) // t is the Ptr< recob::Track >::key()
        {
            auto vout = fMVAWriter.template getOutput<recob::Hit>(trkHitPtrList[t],
                [&](art::Ptr<recob::Hit> const & ptr) { return (float)hitInFA[ptr.key()]; });
    ...
    EmTrack<N>::create_hitmap(std::vector<art::Ptr<recob::Hit>> const & hitPtrList) const {
...
template<size_t N>
std::vector<char>
EmTrack<N>::classify_hits(art::Event const  & evt, EmTrack::cryo_tpc_view_keymap const & hitMap, std::vector< art::Ptr<recob::Hit>> const & hitPtrList)
{
    auto hitID = fMVAWriter.template initOutputs<recob::Hit>(fHitModuleLabel, hitPtrList.size(), fPointIdAlgTool->outputLabels());
...
template <size_t N>
EmTrack<N>::EmTrack(EmTrack::Config const& config, std::string const& module_label, art::ProducesCollector & collector) :
	    ...
	    fViews(config.Views()),
	    fNewClustersTag(module_label, "", art::ServiceHandle<art::TriggerNamesService const>()->getProcessName())
{
